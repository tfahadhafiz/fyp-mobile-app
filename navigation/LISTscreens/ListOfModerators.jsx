...
const ListOfModerators = () => {
  ...
  useEffect(() => {
    fetch('http://3.26.23.172/view_users')
      ...
      .then(data => {
        const filteredData = data.filter(item => item.account_type === 'moderator');
        const sortedData = filteredData.sort((a, b) => a.name.localeCompare(b.name));
        setModerators(sortedData);
      })
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        <Text style={styles.topBarText}>Moderators List</Text>
      </View>
...
const ListOfModeratorsScreen = ({ navigation }) => {
  useEffect(() => {
...
export default ListOfModeratorsScreen;
